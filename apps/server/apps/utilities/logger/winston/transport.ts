...
import { Logging } from "@interfaces/logging"
import { pad } from "@utilities/string/pad"
...
function loggerParser(data) {
  const parsedResponseHorasLogging: Logging = data.message
  ...
  if (parsedResponseHorasLogging?.ip) {
    const account = pad(
      sPad.account,
      `${parsedResponseHorasLogging?.account?.last_name ?? "SYSTEM"}, ${parsedResponseHorasLogging?.account?.first_name ?? "SYSTEM"}`,
      false,
    )
    const httpCode = pad(
      sPad.httpCode,
